...
pub mod test {
    ...
    fn db_backup_and_recover() {
        ...
        inputs.push(create_mempool_input_with_pay(&keypair));
        inputs.push(create_mempool_input_with_pay(&keypair));
        inputs.push(create_mempool_input_with_pay(&keypair));
        mem_pool.add(inputs, inserted_block_number, inserted_timestamp);
    ...
    fn create_mempool_input_with_pay(keypair: &KeyPair) -> MemPoolInput {
        let signed = create_signed_pay(&keypair);
