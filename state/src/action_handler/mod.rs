...
pub trait ActionHandler: Send + Sync {
    ...
    fn query(&self, key_fragment: &[u8], state: &TopLevelState) -> StateResult<Option<Vec<u8>>> {
        let key = ActionDataKeyBuilder::key_from_fragment(key_fragment);
...
impl ActionDataKeyBuilder {
    fn prepare() -> ActionDataKeyBuilder {
    ...
    pub fn new(fragment_length: usize) -> ActionDataKeyBuilder {
        let mut builder = Self::prepare();
...
mod tests {
    ...
    fn action_data_key_builder_raw_fragment_and_list_are_same() {
        let key1 =
            ActionDataKeyBuilder::new(3).append(&"key").append(&"fragment").append(&"has trailing list").into_key();

        let mut rlp = RlpStream::new_list(3);
        rlp.append(&"key").append(&"fragment").append(&"has trailing list");
        let key2 = ActionDataKeyBuilder::key_from_fragment(rlp.as_raw());
        assert_eq!(key1, key2);
    }
}
